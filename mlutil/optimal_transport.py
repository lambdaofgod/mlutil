...
def get_stem_vectors(filtered_stems, keyed_vectors):
    return np.vstack(
        [
            ...
            for stem_list in filtered_stems if len(stem_list) > 0
        ]
    )


def get_word_vector_optimal_transport(word_vectors1, word_vectors2, ot_method=ot.sinkhorn, reg=0.01, normalize_dists=True):
...
def get_optimal_transport_result(text1, text2, vectorizer, ot_method=ot.sinkhorn, **kwargs):
    word_vectors1 = vectorizer(text1)
    word_vectors2 = vectorizer(text2)
    ot_matrix, ot_cost = get_word_vector_optimal_transport(word_vectors1, word_vectors2, ot_method=ot_method, **kwargs)
...
def get_ot_result_df(text1, text2, vectorizer, stem_extractor=None, **kwargs):
    ...
    ot_matrix, ot_cost = get_optimal_transport_result(text1, text2, vectorizer, **kwargs)
