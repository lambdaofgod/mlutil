    ...
    def optimize(
        ...
        for _iter in iterator:
            ...
            if verbose and _iter % log_period == 0:
                ...
                plt.scatter(plt_values_selected[:,0], plt_values_selected[:, 1], c='red', label='selected with nondominated sort');
...
def crowding_distance_assignment(pop):
    ...
    for m in range(n_criterions):
        ...
        for i in range(1, size-1):
            ...
            df = fmax - fmin + 1e-8
            dist_increment = -pop[sorted_pop_indices[i-1],m] + pop[sorted_pop_indices[i+1],m]
            crowd_dist[sorted_pop_indices[i]] += dist_increment
    return crowd_dist


@numba.jit(nopython=True)
def select_from_front(n_selected, front_indices, ranks, crowding_distances):
    ...
    if n_rank_selected < n_selected:
        ...
        crowding_distance_sorted_indices = crowding_distances[rank_tied_indices][::-1].argsort()
        selected_indices[n_rank_selected:] = rank_tied_indices[crowding_distance_sorted_indices][:n_selected-n_rank_selected]
...
@numba.jit(nopython=True)
def nsga_selection(n_selected, objective_values):
    ...
    for i in range(len(fronts)):
        ...
        if select_counter + len(front) > n_selected:
            ...
            crowding_dist = crowding_distance_assignment(front_objective_values)
            selected_objective_values_indices[select_counter:] = select_from_front(n_selected-select_counter, front, front_ranks, crowding_dist) 
            select_counter = n_selected
        else:
            selected_objective_values_indices[select_counter:select_counter+len(front)] = front
